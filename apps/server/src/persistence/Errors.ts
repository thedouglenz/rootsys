...
export class ForeignMigrationLineageError extends Schema.TaggedErrorClass<ForeignMigrationLineageError>()(
  ...
  override get message(): string {
    return [
      ...
      "trellis keeps its own migration lineage, so a state.sqlite written by upstream T3 Code cannot",
...
export type McpCredentialRepositoryError = PersistenceSqlError | PersistenceDecodeError;
