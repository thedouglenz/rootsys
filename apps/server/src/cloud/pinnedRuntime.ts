 ...
 * switching over, never `npx <package>`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", packageJson.name, "dist", "bin.mjs"),
...
/**
 * Installs `<package>@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", packageJson.name, "dist", "bin.mjs"),
    ...
    yield* runner
      .run({
        ...
        args: [
          ...
          "--no-fund",
          "--no-audit",
          `${packageJson.name}@${input.version}`,
        ],
