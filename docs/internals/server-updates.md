...
The launcher is the only runtime writer of `service-state.json`. `trellis service install` and
`trellis service update` may replace the launcher and state while the unit is stopped. Server children
...
```sh
trellis@<version> service update
```
