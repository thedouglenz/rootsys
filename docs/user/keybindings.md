...
The same configuration lives in `~/.rootsys/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. rootsys writes the built-in defaults into that file on
