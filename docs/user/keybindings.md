...
The same configuration lives in `~/.trellis/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. trellis writes the built-in defaults into that file on
