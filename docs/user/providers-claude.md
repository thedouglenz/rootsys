...
This guide is for people who want to use more than one Claude setup in trellis. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install trellis](./install.md).
...
In trellis Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means trellis uses Claude Code's normal config directory.

When you set this field, trellis points Claude Code at that directory with the
...
trellis looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where trellis looks.

Then add another Claude provider in trellis:
...
trellis only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so trellis keeps separate config directories isolated
...
Add or edit a Claude provider in trellis Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. trellis stores the value as a server secret and does not
...
trellis does not need a special Claude Code Router provider. Treat the router as a Claude
