...
If reordering is unavailable for one environment, update the rootsys server running in that
...
pill** fallback because their colors are not controlled by rootsys.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While rootsys is generating it, the action reads **Regenerating…** and cannot
