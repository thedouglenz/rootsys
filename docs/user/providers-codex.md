...
This guide is for people who want to use more than one Codex account in trellis. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install trellis](./install.md).
...
In trellis Settings, name it something obvious:
...
In trellis Settings, add another Codex provider:
...
trellis shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, trellis treats it
