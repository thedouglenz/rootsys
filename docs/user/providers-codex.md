...
This guide is for people who want to use more than one Codex account in rootsys. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install rootsys](./install.md).
...
In rootsys Settings, name it something obvious:
...
In rootsys Settings, add another Codex provider:
...
rootsys shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, rootsys treats it
